...
export function getResponsiveClass(baseClass: string, deviceType: DeviceType): string {
  const deviceClasses = {
...
export function getResponsiveTextSize(size: 'xs' | 'sm' | 'base' | 'lg' | 'xl' | '2xl'): string {
  const textSizes = {
...
export function getResponsiveSpacing(type: 'p' | 'px' | 'py' | 'm' | 'mx' | 'my'): string {
  const spacingTypes = {
    p: 'responsive-p',
    px: 'responsive-px',
    py: 'responsive-py',
...
export function getResponsiveWidth(size: 'full' | 'auto' | 'sm' | 'md' | 'lg'): string {
  const widthSizes = {
