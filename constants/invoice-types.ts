...
export const INVOICE_FILTER_OPTIONS = [
  ...
  { key: "purchase", label: "فواتير الشراء", color: "primary", value: INVOICE_TYPES.PURCHASE },
  { key: "sales", label: "فواتير البيع", color: "success", value: INVOICE_TYPES.SALES },
  { key: "purchase_return", label: "مردود الشراء", color: "warning", value: INVOICE_TYPES.PURCHASE_RETURN },
  { key: "sales_return", label: "مردود البيع", color: "danger", value: INVOICE_TYPES.SALES_RETURN },
] as const;

// دالة مساعدة للحصول على اسم النوع
export function getInvoiceTypeLabel(transType: number): string {
  return INVOICE_TYPE_LABELS[transType as keyof typeof INVOICE_TYPE_LABELS] || "غير محدد";
}

// دالة مساعدة للحصول على لون النوع
export function getInvoiceTypeColor(transType: number): string {
  return INVOICE_TYPE_COLORS[transType as keyof typeof INVOICE_TYPE_COLORS] || "default";
}
