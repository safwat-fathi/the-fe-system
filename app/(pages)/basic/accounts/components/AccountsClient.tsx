...
export default function AccountsClient({ initialAccounts, initialCurrencies }: AccountsClientProps) {
  ...
  const removeAccountFromTree = (tree: Account[], accountId: number): Account[] => {
    return tree.map(node => ({
      ...node,
      children: node.children 
        ? removeAccountFromTree(node.children.filter(child => child.id !== accountId), accountId)
        : undefined
  ...
  const generateAccountId = (parentId: number | null): string => {
    ...
    if (parentAccount && parentAccount.acc_level < 4) {
      ...
      newSuffix = (Math.max(...siblingNumbers, 0) + 1).toString().padStart(4, "0");
    } else {
      // للمستويات الأخرى
      newSuffix = (siblingCount + 1).toString();
    }
    ...
    try {
      const newAccount: any = {
        ...
        acc_level: formData.parent ? 
          (flattenAccountTree(accounts).find(a => a.id === formData.parent)?.acc_level || 0) + 1 : 1
      };
      ...
      if (!isEdit) {
        ...
        const optimisticAccount = { ...newAccount, id: optimisticId, children: [] };
        const flatAccounts = flattenAccountTree(accounts);
