...
interface GoldInvoice2PageProps {
  params: Promise<{
    id: string;
  }>;
}

export default function GoldInvoice2Page({ params }: GoldInvoice2PageProps) {
  ...
  // تحديد ما إذا كان هذا إنشاء جديد أم تحميل فاتورة موجودة
  ...
  // تحميل الفاتورة إذا كان ID موجود
  useEffect(() => {
    if (!isNewInvoice && id) {
  ...
  const loadInvoice = async (invoiceId: number) => {
    try {
      setIsLoading(true);
      // TODO: استبدال هذا بـ API call حقيقي لتحميل الفاتورة
      // const invoiceData = await fetchData(`${API_BASE_URL}invoices/${invoiceId}`);

      // محاكاة تحميل البيانات
      toast.success(`تم تحميل الفاتورة رقم ${invoiceId}`);

      // هنا ستقوم بتحميل بيانات الفاتورة وتحديث الحالة
      ...
      router.push("/dashboard/forms/invoices/Gold_invoice2/new");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const saveInvoice = async () => {
    ...
    try {
      ...
      if (isNewInvoice) {
        ...
        // const newInvoice = await apiFetch(API_ENDPOINTS.CREATE_INVOICE, {
        //   method: "POST",
        //   body: JSON.stringify(invoiceData)
        // });

        toast.success("تم حفظ الفاتورة بنجاح");
        // إعادة توجيه إلى صفحة الفاتورة الجديدة
        // router.push(`/dashboard/forms/invoices/Gold_invoice2/${newInvoice.id}`);
      } else {
        // تحديث فاتورة موجودة
        // TODO: استبدال هذا بـ API call حقيقي
        // await apiFetch(`${API_ENDPOINTS.UPDATE_INVOICE}/${id}`, {
        //   method: "PUT",
        //   body: JSON.stringify(invoiceData)
        // });
