      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        <div className="border-b pb-4 mb-6">
          <h1 className="text-2xl font-bold text-gray-800">
            فاتورة رقم: {invoice.inv_id}
          </h1>
          <p className="text-gray-600">
            تاريخ الفاتورة:{" "}
            {new Date(invoice.inv_date).toLocaleDateString("ar-EG")}
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <div>
            ...
            <div className="space-y-2">
              <p>
                ...
                {invoice.inv_amt || "غير محدد"}
              </p>
              <p>
                <span className="font-medium">الصافي:</span>{" "}
                {invoice.inv_net || "غير محدد"}
              </p>
              {invoice.gold_price && (
                <p>
                  <span className="font-medium">سعر الذهب:</span>{" "}
                  {invoice.gold_price}
                </p>
              )}
            </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              <p>
                <span className="font-medium">رقم التعريف:</span> {invoice.id}
              </p>
            </div>
          </div>
        </div>
                  ...
                  {details.map((detail) => (
                    <tr key={detail.id} className="border-b">
                      <td className="py-2 px-4">
                        {detail.item_name || detail.item_code || "غير محدد"}
                      </td>
