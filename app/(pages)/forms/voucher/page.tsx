...
export default function VoucherEntryPage() {
  ...
  const loadVoucher = async (id: number) => {
    try {
      ...
      if (vouchersResponse.success && vouchersResponse.data && Array.isArray(vouchersResponse.data)) {
        const targetVoucher = vouchersResponse.data.find((v: any) => v.id === id);
        ...
        if (targetVoucher) {
          ...
          const voucherIndex = vouchersResponse.data.findIndex((v: any) => v.id === id);
  ...
  const navigateToVoucher = (direction: 'first' | 'prev' | 'next' | 'last') => {
    ...
    if (targetVoucher) {
      router.push(`/forms/voucher?id=${targetVoucher.id}`);
    }
  };
  ...
  const saveVoucher = async () => {
    ...
    try {
      ...
      if (masterId) {
        router.push(`/forms/voucher?id=${masterId}`);
      }
    } catch (error) {
  ...
  const createFromPrevious = () => {
    ...
    router.push(`/forms/voucher?id=${selectedVoucher.vouch_id}&copy=true`);
  };
  ...
  const handleSearch = () => {
    if (searchTerm) {
      ...
      if (foundVoucher) {
        router.push(`/forms/voucher?id=${foundVoucher.id}`);
      }
    }
  };
