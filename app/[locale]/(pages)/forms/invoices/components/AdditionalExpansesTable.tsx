...
>(function AdditionalExpansesTable({ accounts, isEditing, initialRows }, ref) {
  ...
  const loadAccountOptions = useCallback(
    async (
      ...
      additional: AccountSelectAdditional | undefined,
    ) => {
  ...
  useImperativeHandle(ref, () => ({
    ...
    saveRows: async ({
      ...
      try {
        ...
        const deletionsSuccess = await processDeletionsList();

        if (!deletionsSuccess) return false;

        const rowsSuccess = await processRowsList(
          invoiceId,
          transType,
          companyId,
          userId,
  ...
  const processAdditionalExpenseDeletion = async (
    serverId: number,
  ): Promise<boolean> => {
  ...
  const updateAdditionalExpense = async (
    row: AdditionalExpanseRow,
    invoiceId: number,
    transType: number,
    companyId: number,
  ...
  const createAdditionalExpense = async (
    row: AdditionalExpanseRow,
    invoiceId: number,
    transType: number,
    companyId: number,
  ...
  const processDeletionsList = async (): Promise<boolean> => {
    for (const serverId of deletedServerIds) {
      if (!(await processAdditionalExpenseDeletion(serverId))) {
        return false;
      }
    }
  ...
  const processRowsList = async (
    ...
    for (const row of rows) {
      ...
      if (row.serverId) {
        const success = await updateAdditionalExpense(
          row,
          invoiceId,
          transType,
          companyId,
        ...
        const createdId = await createAdditionalExpense(
          row,
          invoiceId,
          transType,
          companyId,
