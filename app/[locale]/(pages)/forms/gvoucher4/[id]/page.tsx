...
export const metadata: Metadata = {
  title: "عرض سند قبض عميل - NafeesWeb",
  description: "عرض وتعديل سند القبض للعميل",
};
...
const getVoucherById = cache(async (voucherId: number) => {
  try {
    ...
    const vouchersResponse = await voucherService.getAll({
      xvouch_type: "4", // سند القبض للعميل فقط
    });
...
export default async function CustomerReceiptVoucherEditPage({
  params,
  searchParams,
}: {
  ...
  const goldDetails: GVoucherDetail[] = goldDetailsData.map((detail: any) => {
    ...
    return {
      ...
      box_name: box?.cust_name || detail.box_name || "",
